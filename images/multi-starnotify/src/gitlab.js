...
exports.processMessage = function processMessage(data) {
  ...
  case 'Merge Request Hook':
    var {object_attributes, changes} = payload;
    ...
    if (!action) break;
    if (!isActionRelevant(action)) {
      console.log('Ignoring irrelevant MR action', action);
      return;
    }
    ...
    notify(channel,
        ...
        action+" "+
        interjection+
        "MR \x02!"+object_attributes.iid+"\x02"+suffix+": "+
  ...
  // notify(channel, "[\x0313"+hookSource+"\x0F] "+
  //        "Got Gitlab event of unhandled type: " + eventType);
